...
on resizeWindow(appName)
	-- Check if app is not added to the property list,
	...
	if listOfSmallApps contains appName then
		...
		if width of standardSizeSmall is less than or equal to 1 then
			set preferredWidth to (width of getDesktopBounds()) * (width of standardSizeSmall)
		else
			-- Otherwise, treat it as an absolute size
			set preferredWidth to width of standardSizeSmall
		end if

		if height of standardSizeSmall is less than or equal to 1 then
			set preferredHeight to (height of getDesktopBounds()) * (height of standardSizeSmall)
		else
			set preferredHeight to height of standardSizeSmall
		end if
	else if listOfLargeApps contains appName then
		if width of standardSizeLarge is less than or equal to 1 then
			set preferredWidth to (width of getDesktopBounds()) * (width of standardSizeLarge)
		else
			set preferredWidth to width of standardSizeLarge
		end if

		if height of standardSizeLarge is less than or equal to 1 then
			set preferredHeight to (height of getDesktopBounds()) * (height of standardSizeLarge)
		else
			set preferredHeight to height of standardSizeLarge
		end if
	else if listOfXLargeApps contains appName then
		if width of standardSizeXLarge is less than or equal to 1 then
			set preferredWidth to (width of getDesktopBounds()) * (width of standardSizeXLarge)
		else
			set preferredWidth to width of standardSizeXLarge
		end if

		if height of standardSizeXLarge is less than or equal to 1 then
			set preferredHeight to (height of getDesktopBounds()) * (height of standardSizeXLarge)
		else
			set preferredHeight to height of standardSizeXLarge
		end if
    else
        if width of standardSizeSmall is less than or equal to 1 then
            set preferredWidth to (width of getDesktopBounds()) * (width of standardSizeSmall)
	...
	try
		...
		set appW to (item 1 of appBounds) + preferredWidth
		set appH to (item 2 of appBounds) + preferredHeight
	...
	on error
		...
		try
			set newBounds to {preferredWidth, preferredHeight}
